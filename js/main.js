...
// Expandable Recommendations functionality
function toggleRecommendation(button) {
    const card = button.closest('.reco-card');
    const fullReco = card.querySelector('.full-recommendation');
    const isExpanded = fullReco.classList.contains('expanded');

    if (isExpanded) {
        // Collapse
        fullReco.classList.remove('expanded');
        button.classList.remove('expanded');
        button.innerHTML = `Read Full Recommendation
            <svg fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd"/>
            </svg>`;
    } else {
        // Expand
        fullReco.classList.add('expanded');
